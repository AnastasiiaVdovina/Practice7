...
### Рішення
Програма складається з кількох функцій.
Основна функція main виконує єдину дію — викликає list_executables_in_current_dir(), яка організовує всю роботу програми.
Вона відкриває поточну директорію за допомогою opendir(".") і проходить по кожному елементу каталогу, використовуючи цикл з readdir(). Для кожного файлу вона будує повний шлях (тобто "./ім’я_файлу") та викликає функцію process_file, яка виконує основну логіку обробки окремого файлу.

Функція process_file в свою чергу перевіряє, чи вказаний файл є звичайним файлом (не директорією, не символьним посиланням тощо) і чи має власник (поточний користувач) право на виконання цього файлу(через stat). Також у цій перевірці використовується допоміжна функція is_executable_by_user, яка перевіряє, чи встановлений біт S_IXUSR у полі st_mode структури stat, що вказує на дозвіл виконання файлу для власника.

Якщо файл задовольняє ці умови, process_file виводить його назву на екран та запитує у користувача, чи слід надати іншим користувачам право на читання цього файлу. Якщо зміна прав пройшла успішно, користувач отримує підтвердження, інакше виводиться повідомлення про помилку.

Результат:

![image](https://github.com/user-attachments/assets/0f87e21d-cd0e-4693-bbf7-1aee7f2efb6a)
...
### Рішення
Це було непросте завдання, але воно реалізовано за допомогою тих самих засобів і функцій що і в попередніх завданнях, окрім lstat, inode, dev.
Загальний алгоритм роботи наступний:
У функції main викликається функція traverse, якій передається шлях до каталогу і початкова глибина обходу.
Далі функція traverse намагається відкрити вказану директорію за допомогою opendir. Відбувається поетапне зчитування вмісту директорії через readdir. 
Для кожного елемента будується повний шлях, який передається у функцію lstat. Вона повертає інформацію про файл або директорію, не переходячи за символічним посиланням. Це дозволяє програмі обробляти симлінки без ризику зациклення. Якщо виклик lstat завершується з помилкою, виводиться повідомлення, і обробка переходить до наступного елемента. Якщо ж інформацію отримано успішно, програма виводить ім’я файлу або директорії з відповідним відступом, який залежить від глибини поточного рівня.

Далі, якщо виявлено, що елемент є символічним посиланням, програма не переходить всередину, а переходить до наступного об’єкта. Якщо ж це директорія, відбувається перевірка на повторне відвідування за допомогою функції already_visited, яка використовує значення inode (унікальний ідентифікатор об’єкта у файловій системі) і dev (ідентифікатор пристрою). 

Окремо встановлено максимальну глибину обходу (MAX_DEPTH), яка запобігає надто глибокому рекурсивному спуску, що може викликати переповнення стеку. 

Результат:

![image](https://github.com/user-attachments/assets/ac22b068-4c13-4997-bdf6-7aed715f4b80)
